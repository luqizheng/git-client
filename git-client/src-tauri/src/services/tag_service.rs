...
pub fn list_tags(repo: &Repository) -> Result<Vec<Tag>, AppError> {
    ...
    for name in names.iter() {
        if let Some(name) = name {
            let target = repo.revparse_single(name)?.id();
            let tag_result = repo.find_tag(target);
            let message = tag_result.as_ref().ok().and_then(|t| t.message().map(|s| s.to_string()));
            let tagger = tag_result.as_ref().ok().and_then(|t| {
