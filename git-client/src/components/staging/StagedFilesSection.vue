  ...
  <div class="staged-section">
    <div class="section-header" @click="expanded = !expanded">
      <div class="header-left">
        ...
        <span class="section-title">Staged Files ({{ files.length }})</span>
      </div>
      <div v-if="files.length > 0 && expanded" class="header-actions">
        <button class="action-btn unstage-all-btn" @click.stop="$emit('unstage-all')" title="Unstage All">−</button>
      </div>
    </div>
    <div v-show="expanded">
      <div
      ...
      >
        ...
        <div class="file-actions">
          <button class="file-action-btn unstage-btn" @click.stop="$emit('unstage', file.path)" title="Unstage">−</button>
        </div>
      </div>
      <div v-if="files.length === 0" class="empty-hint">No staged changes</div>
    </div>
  </div>
</template>
